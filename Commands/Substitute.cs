...
internal class Substitute : CommandBase
{
    ...
    public override async Task RegisterCommand(DiscordSocketClient client, SocketGuild guild)
    {
        await guild.CreateApplicationCommandAsync(new SlashCommandBuilder()
            ...
            .AddOption(WEEKNUMBEROPTIONNAME, ApplicationCommandOptionType.Integer, "The week this sub will take place. Default to the next week.")
            .Build());
    }
    ...
    internal async Task CommandExecuted(SocketSlashCommand slashCommand)
    {
        ...
        if (match.Substitutions.Any(s => s.SubstituteID == sub.Id))
        {
            await slashCommand.ModifyOriginalResponseAsync((mp) =>
            {
                mp.Content = "That sub is already substituting this week!";
            });
            await dBContext.DisposeAsync();
            return;
        }
